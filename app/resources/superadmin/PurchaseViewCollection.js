...
const getModelObject = (data) => {
  ...
  for (let i = 0; i < data.purchaseProducts.length; i++) {
    ...
    for (let x = 0; x < item.purchaseMaterials.length; x++) {
      ...
      materials.push({
        ...
        discount_amount_display: displayAmount(
          thisM.discount_amount,
          false,
          true,
          false,
        ),
    ...
    if (materials.length == 1) {
      total_weight =
        weightFormat(materials[0].weight).toFixed(3) +
        " " +
        materials[0].unit_name;
    } else {
      total_weight =
        materials.length > 0
          ? weightFormat(item.total_weight).toFixed(3) + materials[0].unit_name
          : weightFormat(item.total_weight).toFixed(3) + " gm";
    }
    let sub_total = !isEmpty(data.sale_id)
      ? displayAmount(item.sub_price)
      : displayAmount(
          parseFloat(item.sub_price) + parseFloat(item.making_charge),
        );
    products.push({
      ...
      sub_category_id:
        item.product && item.product.sub_category
      ...
      sub_category_hsn:
        item.product && item.product.sub_category
          ? item.product.sub_category.hsn_code
          : "",
  ...
  if (products) {
    for (let i = 0; i < products.length; i++) {
      ...
      if (
        ...
        productSelected.sub_category_id != "" &&
        productSelected.sub_category_id != null
      ) {
        //productSelected = productSelected[0];
        if (
          productSelected.sub_category_id &&
          typeof subCatWiseProducts[productSelected.sub_category_id] ===
            "undefined"
        ) {
    ...
    for (let subCatIndex in subCatWiseProducts) {
      ...
      let subCatWiseProductHSNs = subCatWiseProduct.map(
        (itm) => itm.product.product_code,
      );
      subCatWiseProductHSNs = subCatWiseProductHSNs
        .filter((item, index) => subCatWiseProductHSNs.indexOf(item) === index)
        .join(", ");

      /* subCatWiseProductMaterials */
      let productTax = 0.0;
      let productTaxableAmount = 0.0;
      let subCatWiseProductMaterials = [];
      for (let i = 0; i < subCatWiseProduct.length; i++) {
        const pItem = subCatWiseProduct[i].product;
        productTax = pItem.taxDetails ? parseFloat(pItem.taxDetails.igst) : 0.0;
        productTaxableAmount += parseFloat(
          pItem.sub_total.replace(/[^\d]/, ""),
        );
        for (let j = 0; j < pItem.materials.length; j++) {
          const mItem = pItem.materials[j];
          if (
            typeof subCatWiseProductMaterials[mItem.material_id] === "undefined"
          ) {
            ...
            subCatWiseProductMaterials[mItem.material_id].weight += parseFloat(
              mItem.weight,
            );
          }
        }
      }
      subCatWiseProductMaterials = subCatWiseProductMaterials.filter(
        (itm) => itm != null,
      );
  ...
  let company_name =
    data.supplier && data.supplier.company_name
  ...
  const supplier_gst =
    data.supplier && data.supplier.gst != null && data.supplier.gst != ""
      ? data.supplier.gst
      : "";
  const purchase_by_gst =
    data.purchaseBy && data.purchaseBy.gst != null && data.purchaseBy.gst != ""
      ? data.purchaseBy.gst
      : "";

  const supplier_gst_state =
    supplier_gst != "" ? supplier_gst.substring(0, 2) : "";
  const purchase_by_gst_state =
    purchase_by_gst != "" ? purchase_by_gst.substring(0, 2) : "";
  ...
  return {
    ...
    supplier_details: {
      ...
      address:
        data.supplier && data.supplier.address ? data.supplier.address : "",
      city: data.supplier && data.supplier.city ? data.supplier.city : "",
      pincode:
        data.supplier && data.supplier.pincode ? data.supplier.pincode : "",
      user_name: user_name,
      bank_name:
        data.supplier && data.supplier.bank_name ? data.supplier.bank_name : "",
      bank_account_no:
        data.supplier && data.supplier.bank_account_no
          ? data.supplier.bank_account_no
          : "",
      bank_ifsc:
        data.supplier && data.supplier.bank_ifsc ? data.supplier.bank_ifsc : "",
    },
    sale: data.sale,
    ...
    purchase_by_details: {
      id: data.user_id,
      company_name:
        data.purchaseBy && data.purchaseBy.company_name
          ? data.purchaseBy.company_name
          : "",
      gst: data.purchaseBy && data.purchaseBy.gst ? data.purchaseBy.gst : "",
      address:
        data.purchaseBy && data.purchaseBy.address
          ? data.purchaseBy.address
          : "",
      city: data.purchaseBy && data.purchaseBy.city ? data.purchaseBy.city : "",
      pincode:
        data.purchaseBy && data.purchaseBy.pincode
          ? data.purchaseBy.pincode
          : "",
      user_name: data.purchaseBy ? data.purchaseBy.name : "",
      bank_name:
        data.purchaseBy && data.purchaseBy.bank_name
          ? data.purchaseBy.bank_name
          : "",
      bank_account_no:
        data.purchaseBy && data.purchaseBy.bank_account_no
          ? data.purchaseBy.bank_account_no
          : "",
      bank_ifsc:
        data.purchaseBy && data.purchaseBy.bank_ifsc
          ? data.purchaseBy.bank_ifsc
          : "",
    },
    added_by_details: {
      id: data.added_by,
      company_name:
        data.addedBy && data.addedBy.company_name
          ? data.addedBy.company_name
          : "",
      ...
      bank_name:
        data.addedBy && data.addedBy.bank_name ? data.addedBy.bank_name : "",
      bank_account_no:
        data.addedBy && data.addedBy.bank_account_no
          ? data.addedBy.bank_account_no
          : "",
      bank_ifsc:
        data.addedBy && data.addedBy.bank_ifsc ? data.addedBy.bank_ifsc : "",
    },
    ...
    accept_declined_at: data.accept_declined_at
      ? formatDateTime(data.accept_declined_at, 7)
      : "",
