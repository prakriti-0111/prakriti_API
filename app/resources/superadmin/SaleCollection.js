...
const getModelObject = (data) => {
  ...
  for (let i = 0; i < data.saleProducts.length; i++) {
    ...
    let total_weight_display =
      materials.length > 0
        ? total_weight + " " + materials[0].unit_name
        : total_weight + " gm";
    ...
    products.push({
      ...
      total_making_charge_discount: priceFormat(
        priceFormat(item.making_charge) -
          priceFormat(item.making_charge_discount_amount),
      ),
    ...
    for (let i = 0; i < subCatWiseProduct.length; i++) {
      ...
      for (let j = 0; j < pItem.materials.length; j++) {
        ...
        if (
          ...
          subCatWiseProductMaterials[mItem.material_id].quantity += parseInt(
            mItem.quantity,
          );
        }
      }
  ...
  let total_tax = priceFormat(
    priceFormat(data.cgst_tax) +
      priceFormat(data.sgst_tax) +
      priceFormat(data.igst_tax),
  );
  let company_name =
    data.user && data.user.company_name ? data.user.company_name : "";
  ...
  const user_gst =
    data.user && data.user.gst != null && data.user.gst != ""
      ? data.user.gst
      : "";
  const sale_by_gst =
    data.saleBy && data.saleBy.gst != null && data.saleBy.gst != ""
      ? data.saleBy.gst
      : "";

  const user_gst_state = user_gst != "" ? user_gst.substring(0, 2) : "";
  const sale_by_gst_state =
    sale_by_gst != "" ? sale_by_gst.substring(0, 2) : "";
  ...
  return {
    ...
    user_details: {
      ...
      bank_account_no:
        data.user && data.user.bank_account_no ? data.user.bank_account_no : "",
    ...
    sale_by_details: {
      id: data.sale_by,
      company_name:
        data.saleBy && data.saleBy.company_name ? data.saleBy.company_name : "",
      ...
      bank_name:
        data.saleBy && data.saleBy.bank_name ? data.saleBy.bank_name : "",
      bank_account_no:
        data.saleBy && data.saleBy.bank_account_no
          ? data.saleBy.bank_account_no
          : "",
      bank_ifsc:
        data.saleBy && data.saleBy.bank_ifsc ? data.saleBy.bank_ifsc : "",
    },
    ...
    return_amount:
      parseFloat(data.return_amount) > 0
        ? displayAmount(data.return_amount)
        : "",
