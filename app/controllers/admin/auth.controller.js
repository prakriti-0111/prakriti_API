...
exports.forgotPasswordSendLink = async(req, res) => {
  try {
    ...
    if (!user || isEmpty(user.email)) {
      addLog(`forgot-password (admin): no account with a registered email matches "${email}" — no mail sent`);
    }
    ...
    if (user && !isEmpty(user.email)) {
      ...
      try {
        ...
        addLog(`forgot-password (admin): sending to ${user.email} failed — ${mailErr}`);
        await UserModel.update(
