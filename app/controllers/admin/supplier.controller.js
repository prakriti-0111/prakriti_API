...
const {
  SupplierCollection,
} = require("@resources/superadmin/SupplierCollection");
...
exports.index = async (req, res) => {
  ...
  if (all == 1) {
    ...
    userModel
      ...
      .then(async (data) => {
        let total_purchase = await PurchaseModel.sum("total_payable", {
          where: { user_id: req.userId, is_approved: { [Op.ne]: 2 } },
        });
        let total_purchase_due = await PurchaseModel.sum("due_amount", {
          where: { user_id: req.userId, is_approved: { [Op.ne]: 2 } },
        });
        let total_purchase_paid = await PurchaseModel.sum("paid_amount", {
          where: { user_id: req.userId, is_approved: { [Op.ne]: 2 } },
        });
...
exports.update = async (req, res) => {
  ...
  if (!admin) {
    return res
      .status(errorCodes.default)
      .send(formatErrorResponse("Supplier not found"));
  }
...
exports.delete = async (req, res) => {
  let roleId = getRoleId("supplier");
  let admin = await userModel.findOne({
    where: { id: req.params.id, role_id: roleId, parent_id: req.userId },
  });
  if (!admin) {
    return res
      .status(errorCodes.default)
