...
exports.store = async (req, res) => {
  ...
  const existing_mobile = await userModel.findOne({
    where: { mobile: data.mobile, role_id: workerRoleId },
  });
  ...
  userModel
    ...
    .then(async (result) => {
      if (!isEmpty(data.profile_image)) {
        let profile_image_data = await base64FileUpload(
          data.profile_image,
          "users"
        );
        userModel.update(
          { profile_image: profile_image_data.path },
          { where: { id: result.id } }
        );
      }

      res.send(
        formatResponse(WorkerCollection(result), "Worker created successfully!")
      );
    })
    .catch((error) => {
      return res
        ...
        .send(
          formatErrorResponse(
            "Worker does not created due to some error" + error
          )
        );
    });
};
...
exports.update = async (req, res) => {
  ...
  userModel
    .update(postData1, { where: { id: req.params.id } })
    .then(async (result) => {
      if (!isEmpty(data.profile_image)) {
    ...
    .catch((error) => {
      return res
        ...
        .send(
          formatErrorResponse(
            "Worker does not updated due to some error" + error
          )
        );
    });
};
...
exports.fetch = async (req, res) => {
  let workerRoleId = getRoleId("worker");
  let worker = await userModel.findOne({
    where: { id: req.params.id, role_id: workerRoleId },
  });
  if (!worker) {
    return res
      .status(errorCodes.default)
  ...
  res.send(
    formatResponse(WorkerCollection(worker), "Worker fetched successfully!")
  );
};
