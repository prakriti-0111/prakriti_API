...
exports.editProfile = async (req, res) => {
  ...
  if (!customer) {
    return res
      ...
      .send(formatErrorResponse("Customer not found"));
  }
  ...
  userModel
    ...
    .then((result) => {
      res.send(
        formatResponse(
          {
            image_url: !isEmpty(profile_image)
              ? getFileAbsulatePath(profile_image)
              : defaultProfileImage(),
          },
    ...
    .catch((error) => {
      return res
        ...
        .send(
          formatErrorResponse(
            "Customer does not updated due to some error" + error
          )
        );
    });
};
