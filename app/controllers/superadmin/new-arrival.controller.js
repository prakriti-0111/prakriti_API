...
exports.store = async (req, res) => {
  ...
  //upload image
  let image = null;
  let result = base64FileUpload(data.image, 'new_arrivals');
  if(result){
    image = result.path;
  }
  ...
  NewArrivalModel.create(postData).then(result => {
...
exports.fetch = async (req, res) => {
  ...
  if (!banner) {
    return res.status(errorCodes.default).send(formatErrorResponse('New arrival banner not found'));
  }
...
exports.update = async (req, res) => {
  ...
  if(!isEmpty(data.image)){
    ...
    let result2 = base64FileUpload(data.image, 'new_arrivals');
    if(result2){
      postData.image = result2.path;
    }
  }

  NewArrivalModel.update(postData, { where: { id: req.params.id } }).then(result => {
...
exports.delete = async (req, res) => {
  ...
  NewArrivalModel.destroy({ where: { id: req.params.id } }).then(result => {
