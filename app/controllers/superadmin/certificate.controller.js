...
} = require("@resources/superadmin/CertificateCollection");
...
exports.store = async (req, res) => {
  ...
  //upload logo
  let logo = null;
  let result = await base64FileUpload(data.logo, "certificates");
  if (result) {
    logo = result.path;
  }
  let website =
  ...
  CertificateModel.create(postData)
    .then((result) => {
      res.send(
        formatResponse(
          CertificateCollection(result),
    ...
    .catch((error) => {
      return res
        ...
        .send(
          formatErrorResponse(
            "Certificate does not created due to some error" + error
          )
        );
    });
};
...
exports.fetch = async (req, res) => {
  ...
  if (!certificate) {
    return res
      .status(errorCodes.default)
      .send(formatErrorResponse("Certificate not found"));
  }
  res.send(
    formatResponse(
...
exports.update = async (req, res) => {
  ...
  if (!isEmpty(data.logo)) {
    removeFile(certificate.logo);
    let result2 = await base64FileUpload(data.logo, "certificates");
    if (result2) {
      postData.logo = result2.path;
    }
  }

  CertificateModel.update(postData, { where: { id: req.params.id } })
    .then((result) => {
      res.send(
        formatResponse(
          CertificateCollection(data),
    ...
    .catch((error) => {
      return res
        ...
        .send(
          formatErrorResponse(
            "Certificate does not updated due to some error" + error
          )
        );
    });
};
...
exports.delete = async (req, res) => {
  ...
  CertificateModel.destroy({ where: { id: req.params.id } })
    .then((result) => {
