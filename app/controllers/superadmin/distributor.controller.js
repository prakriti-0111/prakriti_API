...
const { isEmpty, isArray, priceFormat, arrayColumn } = require("@helpers/helper");
const db = require("@models");
const { Op } = require("sequelize");
const { getRoleId, getNextUserName, getWorkingUserID, getUserColumnValue, isSuperAdmin, isAdmin, isDistributor, isSalesExecutive, avlStockUserIdsNew } = require("@library/common");
...
exports.index = async (req, res) => {
  ...
  if(own == '1'){ // user leg own distributors
    conditions.own = true;
  }else if(own == '0'){ // user leg other distributors
...
exports.store = async (req, res) => {
   ...
   */
  const existOwnDistributer = await userModel.findOne({where: {country_id: data.country_id, state_id: data.state_id, district_id: data.district_id, role_id: roleId, own:1}});
...
exports.update = async (req, res) => {
   ...
   */
  const existUser = await userModel.findOne({where: {role_id: roleId, country_id: data.country_id, state_id: data.state_id, district_id: data.district_id, own:1, id: {[Op.ne]: req.params.id }}});
...
exports.delete = async (req, res) => {
  ...
  if (PurchaseExists) {
    return res.status(errorCodes.default).send(formatErrorResponse('Distributor has purchase record(s).'));
  }
  ...
  if (SaleExists) {
    return res.status(errorCodes.default).send(formatErrorResponse('Distributor has sale record(s).'));
  }
  ...
  if (StockExists) {
    return res.status(errorCodes.default).send(formatErrorResponse('Distributor has stock record(s).'));
  }
