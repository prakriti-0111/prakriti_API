...
const { getRoleId, getCustomRoleIds, getNextUserName, isDistributor, getUserColumnValue, isSalesExecutive, isSuperAdmin, isAdmin } = require("@library/common");
    ...
    if (role_id == 4) {
      if (isDistributor(req)) {
        ...
        let ownDistributors = await userModel.findAll({
          attributes: ["id"],
          where: { role_id: distributorRoleId, own: true, parent_id: req.userId },
        });
        let ownDistributorIds = arrayColumn(ownDistributors, "id");
        for(let i = 0; i < ownDistributorIds.length; i++) { pIds.push(ownDistributorIds[i]); }
        ...
        let distributors = await userModel.findAll({
          attributes: ["id"],
          where: { role_id: distributorRoleId, own: true, parent_id: {[Op.in]: adminIds} },
        });
        let distributorIds = arrayColumn(distributors, "id");
        for(let i = 0; i < distributorIds.length; i++) { pIds.push(distributorIds[i]); }
        ...
        let distributors = await userModel.findAll({
          attributes: ["id"],
          where: { role_id: distributorRoleId, own: true, state_id: state_id, parent_id: req.userId },
        });
        let distributorIds = arrayColumn(distributors, "id");
