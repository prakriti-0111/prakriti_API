...
exports.update = async (req, res) => {
  ...
  try {
    ...
    const trans = await sequelize.transaction(async (t) => {
      ...
      let deleteCount = await ProductMaterialModel.destroy({ where: { product_id: product.id }, transaction: t});
      ...
      if(data.type != "material"){
        for(let i = 0; i < data.materials.length; i++){
          ...
          let result = await ProductMaterialModel.create(mObject, { transaction: t });
      ...
      await ProductSizeMaterialModel.destroy({ where: { product_id: product.id }, transaction: t});
      let size_m_ids = [];
      for(let i = 0; i < data.size_materials.length; i++){
        for(let x = 0; x < data.size_materials[i].materials.length; x++){
          let whereObj = {
          ...
          let result = await ProductSizeMaterialModel.create(thisObj, { transaction: t });
