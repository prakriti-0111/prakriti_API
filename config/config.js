...
const development = createMySqlConfig({
  username: process.env.DB_DEV_USERNAME || process.env.DB_USERNAME || "mysql",
  password:
    process.env.DB_DEV_PASSWORD || process.env.DB_PASSWORD || "prakriti-dev",
  database:
    process.env.DB_DEV_DATABASE || process.env.DB_DATABASE || "prakriti-dev",
  host: process.env.DB_DEV_HOST || process.env.DB_HOST || "194.238.18.233",
  port: process.env.DB_DEV_PORT || process.env.DB_PORT || 3309,
});
...
const production = createMySqlConfig({
  ...
  port: process.env.DB_PROD_PORT || 3307,
});
...
const test = createMySqlConfig({
  ...
  port: process.env.DB_TEST_PORT || 3308,
});
